...
def gcn_stream_monitoring(arguments):
        ...
        for el in pr_i["cmdline"]:
            if el == "start_gcn_stream":
                ...
                table_info = [
                    ...
                    ["create_time", datetime.datetime.fromtimestamp(pr_i["create_time"], paris_tz).strftime("%Y-%m-%d %H:%M:%S")]
                ]
    ...
    gcn_table = [
        ["number of gcn", len(pdf_gcn)]
    ]

    gcn_table += [["number of gcn for {}".format(str(instr)), len(pdf_gcn[pdf_gcn["instruments"] == str(instr)])] for instr in ALL_INSTRUMENTS]
